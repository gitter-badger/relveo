...
public class ChartServiceImpl implements ChartService {
    @Override
    public List<ChartContainer> consumptionCalculator(List<ElectricityIndexStatement> indexStatements) {
        ...
        for (int i = 1; i < indexStatements.size(); i++) {
            ...
            log.debug("Day Consumption of " + indexStatement.getStatementDate().toString() + " is " + dayConsumption);
            ...
            log.debug("Night Consumption of " + indexStatement.getStatementDate().toString() + " is " + nightConsumption);
            nightConsumptionChartValue.add(ChartValue.builder()
                    .name(indexStatement.getStatementDate().toString())
                    .value(indexStatement.getNightIndex() - previousIndexStatement.getNightIndex())
                    .build());
        }
        chartContainers.add(ChartContainer.builder().name("Day Consumption").series(dayConsumptionChartValue).build());
        chartContainers.add(ChartContainer.builder().name("Night Consumption").series(nightConsumptionChartValue).build());
